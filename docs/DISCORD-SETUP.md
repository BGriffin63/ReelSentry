# ReelSentry — Discord Webhook Setup (spec §25)

ReelSentry sends Discord alerts through an **incoming webhook**. There is **no
...
2. Choose (or create) the **channel** for ReelSentry alerts.
3. Open **Channel Settings** (the gear icon next to the channel).
4. Go to **Integrations**.
5. Click **Create Webhook** (or **New Webhook**).
6. Name it **ReelSentry** (optionally set an avatar).
7. Click **Copy Webhook URL**.
8. In Unraid: **Settings → ReelSentry → Notifications → Discord webhook URL**,
   paste the URL.
...
- Footer: `ReelSentry • Monitor every VM`.
...
- ReelSentry stores it in `/boot/config/plugins/reelsentry/secrets.json` with
  `0600` permissions.
...
- If it is ever exposed, **delete/regenerate it in Discord** (Channel → Integrations
  → the webhook → Delete/regenerate) and paste the new one into ReelSentry.
...
| HTTP 429 | You're being rate-limited; ReelSentry backs off automatically. |
