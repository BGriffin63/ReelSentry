#!/usr/bin/env bash
# VM Sentinel — assemble the installable plugin tree into build/stage (spec §20).
# SPDX-License-Identifier: MIT
# Runs on Linux/CI. Copies src/ libraries into the WebGUI package tree so the
# result mirrors exactly what installs to /usr/local/emhttp/plugins/vm.sentinel.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAGE="$ROOT/build/stage"
PLUGDST="$STAGE/usr/local/emhttp/plugins/vm.sentinel"

echo "==> Cleaning stage"
rm -rf "$STAGE"; mkdir -p "$PLUGDST"

echo "==> Copying WebGUI package tree"
cp -a "$ROOT/package/usr/local/emhttp/plugins/vm.sentinel/." "$PLUGDST/"
...
cp "$ROOT/assets/vm-sentinel-128.png" "$PLUGDST/assets/" 2>/dev/null || true
cp "$ROOT/LICENSE" "$PLUGDST/LICENSE"

echo "==> Normalizing permissions"
find "$PLUGDST" -type f -name '*.sh' -exec chmod 755 {} +
chmod 755 "$PLUGDST/hooks/vm-sentinel-hook" "$PLUGDST/services/vm-sentinel-service" 2>/dev/null || true
